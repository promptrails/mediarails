...
const defaultBaseURL = "https://api.lumalabs.ai/dream-machine/v1/generations"

// Provider implements mediarails.Provider for Luma Dream Machine video generation.
...
// New creates a new Luma provider.
...
// For image-to-video, set req.InputURL to the source image.
func (p *Provider) Generate(ctx context.Context, req *mediarails.GenerateRequest) (*mediarails.GenerateResponse, error) {
	...
	if req.InputURL != "" {
		body["keyframes"] = map[string]interface{}{
			"frame0": map[string]interface{}{
				"type": "image",
				"url":  req.InputURL,
			},
		}
	...
	if err != nil {
		return nil, fmt.Errorf("luma: request error: %w", err)
	}
	...
	if resp.StatusCode != http.StatusOK && resp.StatusCode != http.StatusCreated {
		return nil, fmt.Errorf("luma: API error (status %d): %s", resp.StatusCode, string(respBody))
	}
	...
	if err := json.Unmarshal(respBody, &result); err != nil {
		return nil, fmt.Errorf("luma: parse error: %w", err)
	}
	...
	return &mediarails.GenerateResponse{
		...
		Usage: &mediarails.Usage{
			Unit:     "seconds",
			Quantity: 5, // Luma default duration
		},
	}, nil
}
...
func (p *Provider) CheckStatus(ctx context.Context, jobID string) (*mediarails.GenerateResponse, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("luma: status error: %w", err)
	}
	httpReq.Header.Set("Authorization", "Bearer "+p.apiKey)

	resp, err := p.client.Do(httpReq)
	if err != nil {
		return nil, fmt.Errorf("luma: status failed: %w", err)
	}
	defer resp.Body.Close()
	...
	if err := json.Unmarshal(respBody, &result); err != nil {
		return nil, fmt.Errorf("luma: status parse error: %w", err)
	}
	...
	case "completed":
		return &mediarails.GenerateResponse{
			...
			AssetURL:    result.Assets.Video.URL,
	...
	case "failed":
		return &mediarails.GenerateResponse{
			...
			Metadata: map[string]any{"failure_reason": result.FailureReason},
		}, nil
	default:
